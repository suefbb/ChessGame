var childclass=[]
var fast = 1
var pawns = []; var rooks,bishops,knights,cros= []
...
var pieces = [pawns[0],pawns[1],pawns[2],pawns[3],pawns[4],pawns[5],pawns[6],pawns[7],pawns[8],pawns[9],pawns[10],pawns[11],pawns[12],pawns[13],rooks[0],rooks[1],bishops[0],bishops[1],knights[0],knights[1],cros[0]]
let oo = 0
function movepawn(mm,...gf){
    ...
    for(s=2;s<198;s++){
        if(Number(childclass[s].id.slice(1,childclass[s].id.length))==Number(pieces[mm-1].parentElement.id.slice(1,pieces[mm-1].parentElement.id.length))+2 && childclass[s].id[0]==pieces[mm-1].parentElement.id[0]){
            childclass[s].children[0].style.display='block'
        }
        else if(Number(childclass[s].id.slice(1,childclass[s].id.length))==Number(pieces[mm-1].parentElement.id.slice(1,pieces[mm-1].parentElement.id.length))+1 && childclass[s].id[0]==pieces[mm-1].parentElement.id[0]){
            childclass[s].children[0].style.display='block'
        }
